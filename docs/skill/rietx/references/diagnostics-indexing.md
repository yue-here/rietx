...
| `INDEX_SEARCH_INCOMPLETE` | Read "no cell found" as "no cell exists". Only a *completed* exhaustive search says that; this one was stopped by its budget or by a size cap, and `search_complete[system]` says which systems it covered. Raise `budget_seconds` only when the message names the budget (Measured: WP-1449) |
