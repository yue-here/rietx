...
SECTIONS: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    (
        ...
        "(Measured: WP-1407) — `.raw` is written by six unrelated vendors and "
        "`.rd` by two, so dispatch is on content and a refusal message is the "
        "thing to read rather than the suffix: a declined `.raw` names the six "
        "and deliberately picks none, because guessing a vendor is how a reader "
        "returns a plausible wrong pattern. **Handed another program's input "
        "file** — a "
