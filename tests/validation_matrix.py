...
DATASETS: dict[str, Dataset] = {
    ...
    "bethanechol": Dataset(
        ...
        "cross_code"),
    "cr2wo6_hb2a": Dataset(
        "gsas2_hb2a_cr2wo6_4K.dat",
        "HFIR HB-2A constant-wavelength neutron (lambda 2.4067 A) Cr2WO6 at "
        "4 K, below its ordering temperature, and its 150 K partner "
        "gsas2_hb2a_cr2wo6_150K.dat, above it -- the GSAS-II Magnetic-II "
        "tutorial pair, vendored verbatim.  No certified moment exists, so "
...
CLAIMS: tuple[Claim, ...] = (
    # ---- SRM 660c + an injected holder doublet: WP-1103's design case ----
    Claim(
        "test_acceptance_extra_peaks", "test_declaring_the_intruder_recovers_the_clean_cell",
        "srm660c", ("own_result",),
        "declaring a sharp intruder that overlaps sample peaks recovers the "
        "cell the same data gives when the intruder is not there, and "
        "ignoring it does not",
        reference="the *clean* arm of this same fixture, refined by the same "
                  "protocol — not the NIST certificate, because what is being "
                  "measured is the bias an unmodelled overlapping line adds, "
    ...
    Claim(
        "test_acceptance_extra_peaks", "test_the_excluded_regions_alternative_is_quoted_and_never_gated",
        "srm660c", ("own_result", "ceiling"),
        "the excluded-regions alternative is reported beside the declared one, "
        "with its cell and the channels it costs, and neither arm is refused",
        reference="the same clean arm; the ceiling is that nothing anywhere "
                  "gates either choice, which is a property of the code rather "
                  "than of a number",
        measured="exclude +0.6 ppm for 256 of 5332 channels (4.8 %) — i.e. on "
                 "THIS fixture exclusion is as accurate as declaring, and the "
                 "case for declaring is the retained channels and the measured "
                 "intruder, not cell accuracy. A pattern with few reflections "
                 "is where that changes, and LaB6 is not one",
    ),
    Claim(
        "test_acceptance_extra_peaks", "test_the_declared_holder_lines_come_back_where_they_were_put",
        "srm660c", ("own_result",),
        "the declared components recover the injected intruder's position, "
        "area and width, with esds",
        reference="the injected truth, which is known exactly because the "
                  "injection uses the package's own evaluator — so this row is "
    ...
    Claim(
        "test_acceptance_extra_peaks", "test_the_declared_peaks_are_ticks_and_not_background",
        "srm660c", ("identity", "ceiling"),
        "a declared peak's positions join result.ticks and its curve stays out "
        "of result.y_background",
        reference="the member contract's clause 2 (schemas/instrument.py): a "
                  "member's aggregate is declared data, and for a peak it is "
                  "the tick list, not the background",
        measured="4 ticks under the reserved key (two peaks x two emission "
                 "lines); across 3xFWHM of each centre the reported background "
                 "swings < 2 % of the peak standing on it",
    ),
    Claim(
        "test_acceptance_extra_peaks", "test_the_arms_are_plotted_for_inspection",
        "srm660c", ("ceiling",),
        "every arm writes obs/calc/diff and per-intruder zooms for visual "
        "inspection",
        measured="12 PNGs to tests/output/; the ignore arm's residual carries "
                 "the whole injected doublet and the declare arm's is flat",
    ),
    ...
    # ---- the measured blank: the same container, scanned empty (WP-1309) ----
    # The rows above fit the container's halo with three free parameters; these
    # measure it.  11BM_Kapton.xy is run 4736 from the same beamtime with the
    # specimen taken out, committed 2026-09-17 and provenanced in
    # tests/data/README.md.  The protocol is the Chebyshev-3 arm above with the
    # polynomial replaced by the blank plus the same polynomial.
    Claim(
        "test_acceptance_si640c",
        "test_a_measured_blank_beats_the_polynomial_and_the_fitted_hump",
        "si640c", ("own_result", "ceiling"),
        "a background shape somebody measured with the sample out, declared "
        "and not fitted, beats both the polynomial that cannot describe it and "
        "the three-parameter hump that can -- which is the case for scanning a "
        "blank at all",
        reference="this package's own three arms under the one protocol "
                  "(Chebyshev-3, Chebyshev-3 + one hump, Chebyshev-3 + the "
                  "declared blank), scored on ONE weight -- the specimen's own "
                  "sigma -- because a measured background's esds move the "
    ...
    Claim(
        ...
        "si640c", ("own_result",),
        "the fraction of the blank that reached the specimen scan is a "
        "measurement, and it is not 1.0 -- the value this package could "
        "express before the scale field existed",
        reference="issue #171's hand-set scan of the same two files, rerun as "
                  "this module's `blank_scan` fixture: the refined scale is "
                  "checked against the minimum a caller had to find by hand.  "
                  "Not an external number -- the monitor normalisation and the "
    ...
    Claim(
        ...
        "si640c", ("own_result",),
        "the minimum in the scale is *interior*, so an additive polynomial "
        "cannot reach it however many terms it is given -- the whole reason a "
        "measured curve needs a multiplier",
        reference="issue #171's own experiment, rerun in the issue's own "
    ...
    Claim(
        ...
        "si640c", ("characterisation", "own_result"),
        "an additive polynomial and a multiplicative scale share the "
        "background's level, so a longer polynomial walks the measured scale "
        "away while Rwp improves -- a lower Rwp does not make a scale more "
        "nearly measured",
        reference="this module's two free-scale arms, three Chebyshev terms "
                  "against six, scored on one common weight.  The same walk "
    ...
    Claim(
        ...
        "si640c", ("characterisation",),
        "a measured background carries counting statistics, they enter the "
        "weight as sigma^2 + s^2 sigma_f^2, and Rwp is therefore a function of "
        "the declared scale -- so a column of Rwp down a scan of scales is not "
        "a ranking and every comparison needs one common sigma",
    ...
    Claim(
        "test_acceptance_si640c", "test_the_blank_arms_render",
        "si640c", ("ceiling",),
        "both blank arms render, including a window framed below the (111) so "
        "the halo is the only thing in the picture -- a correction that acts "
        "on 5 % of the range can be looked at rather than only summarised",
        reference="existence, not a number -- a ceiling row",
    ...
    # ---- Cr2WO6 on HB-2A: WP-1327's magnetic moment and its null ----
    Claim(
        "test_acceptance_magnetic", "test_the_4k_moment_is_supported_and_lies_along_a",
        "cr2wo6_hb2a", ("characterisation", "ceiling"),
        "below T_N a moment under BNS 58.395 is reported supported, along a, "
        "and removes about half of the nuclear-only misfit",
        reference="no certified moment exists; the band 1.9-2.3 mu_B is an "
                  "envelope around GSAS-II's own 2.121 +/- 0.019 on the "
    ...
    Claim(
        "test_acceptance_magnetic", "test_the_4k_moment_does_not_depend_on_its_in_plane_seed",
    ...
    Claim(
        "test_acceptance_magnetic", "test_the_150k_null_finds_no_moment",
        "cr2wo6_hb2a", ("characterisation",),
        "above T_N the same moment model is reported unsupported and does not "
        "move Rwp -- WP-1327's null, the one check separating no moment from "
...
SUITE_INTROS: dict[str, str] = {
    "test_acceptance_extra_peaks": (
        "WP-1103's design case on real data: the SRM 660c protocol, "
        "contaminated by injecting a two-line holder doublet **onto** two LaB6 "
        "reflections, then refined four ways — clean, ignore, declare, "
        "exclude.  The intruder is synthetic because no committed standard "
        "carries a holder line; the pattern it is injected into is real, which "
        "is what the cell bias has to be measured against, and every arm is "
        ...
        "reflections is where that changes, and LaB6 is not one."
    ),
    ...
    "test_acceptance_magnetic":
        "A magnetic moment on real neutron data, and the refusal of one. "
        "The pair is one specimen either side of its ordering temperature, so "
        "the same moment model is asked twice: below T_N the report calls "
        "the moment supported, along a from either in-plane seed; above it "
